...
import { Trash2, Check, Loader2, Key, Settings2, Cpu, Send } from 'lucide-react';
...
export function ProviderCard({ projectId, provider, config, onSave }: ProviderCardProps) {
  ...
  const handleSave = async (data: ProviderConfigFormData) => {
    ...
    try {
      ...
      toast.success(t('toasts.configSaved'), {
        description: t('toasts.configSavedDescription', { provider: getAIProviderDisplayName(provider) }),
      });
      setIsEditing(false);
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success(t('toasts.configRemoved'), {
        description: t('toasts.configRemovedDescription', { provider: getAIProviderDisplayName(provider) }),
      });
    ...
    try {
      ...
      if (result.success) {
        toast.success(t('toasts.connectionSuccess'), {
          description: t('toasts.connectionSuccessDescription', { provider: getAIProviderDisplayName(provider) }),
        });
      } else {
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card/50 overflow-hidden animate-fade-in-up">
      {/* Header */}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-base truncate">{providerName}</h3>
              {config && !isEditing && (
                <span
                  className={cn(
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSave)} className="space-y-4">
                ...
                render={({ field }) => (
                  <FormItem>
                    <div className="flex items-baseline justify-between">
                      <FormLabel className="text-sm font-medium">{t('provider.form.apiKey')}</FormLabel>
                      {isExisting && (
                        <span className="text-[10px] text-muted-foreground">{t('provider.form.apiKeyKeepCurrent')}</span>
                      )}
                    </div>
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input
                          type="password"
                          placeholder={isExisting ? t('provider.form.apiKeyUpdatePlaceholder') : t('provider.form.apiKeyPlaceholder', { provider: providerName })}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-sm font-medium">{t('provider.form.model')}</FormLabel>
              ...
              <FormField
                ...
                name="isActive"
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-xl border border-border/60 bg-background/30 p-4">
                    <div className="space-y-0.5">
                      <FormLabel className="text-sm font-medium">{t('provider.form.enableProvider')}</FormLabel>
                      <FormDescription className="text-[11px]">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-background/50 border border-border/40 p-3">
                ...
                <p className="text-sm font-medium truncate">{getModelDisplayName(config?.model || '')}</p>
              </div>
              <div className="rounded-xl bg-background/50 border border-border/40 p-3">
                <div className="flex items-center gap-1.5 text-muted-foreground mb-1">
                ...
                <p className="text-sm font-mono font-medium">{config?.keyPrefix}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2">
              <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>{t('provider.removeDialog.title', { provider: providerName })}</AlertDialogTitle>
                    <AlertDialogDescription>
