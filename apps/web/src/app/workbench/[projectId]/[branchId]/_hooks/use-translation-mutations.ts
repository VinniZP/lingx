...
import { ApiError, translationApi, TranslationKey } from '@/lib/api';
...
export function useTranslationMutations({ branchId }: UseTranslationMutationsOptions) {
  ...
  const getTranslationValue = useCallback(
    (key: TranslationKey, lang: string): string => {
  ...
  const handleTranslationChange = useCallback(
    (keyId: string, lang: string, value: string) => {
      ...
      const timeout = setTimeout(async () => {
          ...
          try {
            ...
            if (!validation.valid) {
              ...
              setValidationErrors((prev) =>
                new Map(prev).set(saveKey, validation.error || 'Invalid ICU syntax')
              );
        ...
        try {
          ...
          // Save failed - error handled by mutation's onError
        }
  ...
  const handleApprove = useCallback(
    async (translationId: string, status: 'APPROVED' | 'REJECTED') => {
  ...
  const handleBatchApprove = useCallback(
    async (
      ...
      try {
        ...
        toast.success(
          t('translations.toasts.translationsUpdated', {
  ...
  const batchApproveTranslations = useCallback(
    async (translationIds: string[], status: 'APPROVED' | 'REJECTED') => {
      if (translationIds.length === 0) return;
      try {
        await translationApi.batchApprove(branchId, translationIds, status);
        if (status === 'APPROVED') {
          toast.success(
            t('translations.toasts.translationsApproved', { count: translationIds.length })
          );
        } else {
          toast.success(
            t('translations.toasts.translationsRejected', { count: translationIds.length })
          );
        }
        queryClient.invalidateQueries({ queryKey: ['keys', branchId] });
      } catch (error) {
        if (status === 'APPROVED') {
          toast.error(t('translations.toasts.failedToApproveTranslations'), {
            description: (error as ApiError).message,
          });
        } else {
          toast.error(t('translations.toasts.failedToRejectTranslations'), {
            description: (error as ApiError).message,
          });
        }
      }
  ...
  const handleBulkDelete = useCallback(
    async (selectedKeys: Set<string>, onSuccess: () => void) => {
      if (selectedKeys.size === 0) return;
