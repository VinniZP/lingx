...
export function useRegisterPasskey(options?: { onSuccess?: () => void }) {
  ...
  return useMutation({
    mutationFn: async (name: string) => {
      const { options: regOptions, challengeToken } = await webauthnApi.getRegistrationOptions();
    ...
    onError: (error) => {
      const message =
        error instanceof Error ? error.message : t('security.passkeys.toasts.passkeyAddFailed');
      toast.error(message);
    },
  });
}
...
export function useDeletePasskey(options?: { onSuccess?: () => void }) {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message =
        error instanceof Error ? error.message : t('security.passkeys.toasts.passkeyDeleteFailed');
      toast.error(message);
    },
  });
}
...
export function useGoPasswordless(options?: { onSuccess?: () => void }) {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message =
        error instanceof Error ? error.message : t('security.passkeys.toasts.passwordlessFailed');
      toast.error(message);
    },
  });
}
