...
export function createMockLanguageModel(responses: MockAIResponseConfig[]): LanguageModel {
  ...
  const mockGenerate = vi.fn().mockImplementation(async () => {
    ...
    return {
      ...
      providerMetadata: config.cacheMetrics ? { anthropic: config.cacheMetrics } : undefined,
    };
  });
  ...
  issues: [
    {
      type: 'accuracy',
      severity: 'critical',
      message: 'Translation appears to be AI explanation rather than translation',
    },
  ],
});
...
export function createMockGenerateText(
  responses: MockAIResponseConfig[]
