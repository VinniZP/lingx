 ...
 * Email Service
 *
 * Enhanced email service with queue-based async delivery.
 * Uses React Email for template rendering and nodemailer for sending.
 */
...
export class EmailService {
  ...
  /**
   * Queue an email for async sending via BullMQ.
   * Templates are rendered to HTML before queuing.
   */
  async queueEmail(to: string, subject: string, template: ReactElement): Promise<void> {
    try {
      ...
      this.logger.info({ to, subject }, 'Email queued for delivery');
    } catch (error) {
      this.logger.error({ error, to, subject }, 'Failed to queue email');
      throw error;
    }
  }

  /**
   * Send email synchronously (used by email worker).
   */
  ...
  /**
   * Send email with raw HTML (for legacy compatibility)
   */
  async sendRaw(options: EmailOptions): Promise<void> {
    try {
      await this.transporter.sendMail({
        from: this.fromAddress,
        to: options.to,
  ...
  /**
   * Send email verification link (for email change flow)
   */
